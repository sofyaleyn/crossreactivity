...
> **✅ EXECUTED (2026-07-05).** This plan is now largely done: the offline demo (Phases 0–5) *and* the Phase −1 live screen have both run. The **real-scale validation screen described below as "planned/optional" is COMPLETE** — SHR-1210 × 1,198 ectodomains + ABT-736→PF4 (blind, recovered) + CDR-scramble control, $342.70 of the $500 budget, hit rule unchanged. It is **conditionally validated** (valid-regime ≥150 aa FPR 2.1%, both FZD5/ULBP2 enriched, VEGFR2 missed; bounded flaw: over-docks <150 aa antigens → size-aware confidence gate). Read the plan below for *intent*, but for **what actually happened and the current numbers** use [`data/results/screen/validation_summary.md`](data/results/screen/validation_summary.md) and [`demo/README.md`](demo/README.md) (both authoritative). The cofold method here is current and correct; only the "not yet run / optional" framing is stale.

**Status:** the offline demo (Phases 0–5) is **BUILT and committed** (commit `94df3cb`): `demo/dashboard.html`, `demo/figures/*.png`, `demo/verdict_table.{md,json}`, the `crossflag.demo` package, and `tests/test_demo.py` all exist and pass. **Phase −1 — the live $500 validation screen (below) has since RUN and is validated** (see the banner above). Companion to [`findings.md`](findings.md) (evidence) and [`plan.md`](plan.md) (the productization pipeline — *not* built here).
